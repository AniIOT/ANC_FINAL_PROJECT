...
#Function to load data (Training AND Testing)
def load_X(path):
    X_signals = []
...
#Function to load labels (Training AND Testing)
def load_y(y_path):
    file = open(y_path, 'r')
...
#Define bias matrix: A tensor of the specified shape filled with constant bias values
...
#Restore existing trained CNN weights (If any)
...
for i in range(200):
    ...
    if accuracy_out > best_accuracy:
        saver.save(sess,'./cnn_ckpt/model') #Save model with best accuracy
