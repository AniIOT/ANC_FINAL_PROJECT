...
# define weight matrix: a tensor of the specified shape filled with random
# truncated normal values
def weight_variable(shape):
    initial = tf.truncated_normal(shape, stddev=0.1)
    return tf.Variable(initial)

# define weight matrix: a tensor of the specified shape filled with constant
# bias values
...
# define bias values
batch_size = 512
#create placeholder tensor for data
X_ = tf.placeholder(tf.float32, [None, 6, 128, 1],name='cnn_X')
#create placeholder tensor for labels
...
# create saver variable for tensorflow v1
saver = tf.train.Saver(max_to_keep=1)

# create session
sess = tf.InteractiveSession(config=config)
#run session to initialize global variables
sess.run(tf.global_variables_initializer())

# restore model if previously run
...
for i in range(200):
    ...
    index = list(range(l))
    # shuffle indexes to avoid overfitting
    ...
    for idx in range(batch_idxs):
        ...
        #print(start,end)
        # run batch to get current accuracy and loss
    ...
    # save model if accuracy is higher than the last best accuracy
